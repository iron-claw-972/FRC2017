...
/*----------------------------------------------------------------------------*/
/* This code has been modified by FRC Team 972 for MJPEG server access.       */
/*----------------------------------------------------------------------------*/
...
public class CameraStreamingServer {
	...
	private static CameraStreamingServer server;

	/**
	 * Get the CameraServer instance.
	 */
	public static synchronized CameraStreamingServer getInstance() {
		if (server == null) {
			server = new CameraStreamingServer();
		}
		return server;
	}
	...
	private static final Pattern reMode = Pattern
	...
	private static String videoModeToString(VideoMode mode) {
		return mode.width + "x" + mode.height + " " + pixelFormatToString(mode.pixelFormat) + " " + mode.fps + " fps";
	}
	...
	private CameraStreamingServer() {
			...
			switch (event.kind) {
				case kSourceCreated: {
					...
					try {
						...
						// Do nothing. Let the other event handlers update this if there is an
						// error.
					}
					break;
				}
		...
		m_tableListener = NetworkTablesJNI.addEntryListener(kPublishName + "/", (uid, key, eventValue, flags) -> {
	 ...
	 *
	 * <p>
	 * The first time this overload is called, it calls {@link #startAutomaticCapture(int)} with
	 * device 0, creating a camera named "USB Camera 0". Subsequent calls increment the device
	 * number (e.g. 1, 2, etc).
	 */
	public VideoSink startAutomaticCapture() {
	 ...
	 *
	 * <p>
	 * This overload calls {@link #startAutomaticCapture(String, int)} with a name of "USB Camera
	 * {dev}".
	 *
	 * @param dev
	 *            The device number of the camera interface
	 */
	public VideoSink startAutomaticCapture(int dev) {
		UsbCamera camera = new UsbCamera("USB Camera " + dev, dev);
		return startAutomaticCapture(camera);
	}
	 ...
	 *            Camera
	 */
	public VideoSink startAutomaticCapture(VideoSource camera) {
		addCamera(camera);
	 ...
	 *
	 * <p>
	 * This is only valid to call after a camera feed has been added with startAutomaticCapture() or
	 * addServer().
	 */
